...
// <tr key={p.id} className={selectedPatient?.id === p.id ? 'selected' : ''} onClick={() => setSelectedPatient(p)}>                      <td>{p.id}</td>
//                       <td>{p.firstName}</td>
//                       <td>{p.middleName || ''}</td>
//                       <td>{p.lastName}</td>
...
// export default SearchPatientScreen;   V2 12072026 
...
const SearchPatientScreen = ({ loggedUser, lang = 'en', onClose }) => {
  ...
  const openAvailability = (patientId) => {
    ...
    setSelectedDoctorId('');
    setSelectedRoomId('');
    setAvailabilityDate(new Date().toISOString().slice(0, 10));
          ...
          {loading ? (
            ...
            <div className="search-patient-table-wrapper">
              <table className="search-patient-table">
                ...
                <tbody>
                  {paginatedPatients.map(p => (
                    <tr key={p.id} className={selectedPatient?.id === p.id ? 'selected' : ''} onClick={() => setSelectedPatient(p)}>
                      <td>{p.id}</td>
                      <td>{p.firstName}</td>
                      <td>{p.middleName || ''}</td>
                      <td>{p.lastName}</td>
            ...
            <div className="search-patient-card-grid">
              {paginatedPatients.map(p => (
                <div
                  key={p.id}
                  className={`search-patient-card ${selectedPatient?.id === p.id ? 'selected' : ''}`}
                  onClick={() => setSelectedPatient(p)}
                >
                  <div className="search-patient-card-header">
                    <span className="search-patient-card-id">#{p.id}</span>
        ...
        {showAvailability && (
          <div className="modal-overlay" onClick={() => setShowAvailability(false)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="form-group">
                <label>{t.label.selectDoctor}</label>
                <select
              ...
              <div className="form-group">
                <label>{t.label.selectRoom}</label>
                <select
              ...
              <button
                ...
                style={{ width: '100%' }}
                onClick={() => checkAvailability(selectedDoctorId, selectedRoomId, availabilityDate)}
                disabled={!selectedDoctorId && !selectedRoomId}
              >
                🔍 {t.btn.checkAvailability}
              </button>
