...
// export default DoctorVisitScreen;  14072026  9:40 am
...
const DoctorVisitScreen = ({ doctorId, visitId, initialStatus = 'NEW', username, onClose, language: propLanguage }) => {
  ...
  const startVisit = useCallback(async () => {
    try {
      ...
      await apiFetch(`/api/visits/${visitId}/start`, { method: 'PUT' });
      setCurrentStatus('IN_PROGRESS');
      showToast(t('doctor.visit.started'), 'success');
    } catch (err) {
      setError(t('doctor.visit.startFailed') + ': ' + err.message);
      showToast(t('doctor.visit.startFailed'), 'error');
    } finally {
      setLoading(false);
    }
  }, [visitId, apiFetch, t, showToast]);

  const closeVisit = useCallback(async () => {
    try {
      setLoading(true);
      await apiFetch(`/api/visits/${visitId}/close`, { method: 'PUT' });
      showToast(t('doctor.visit.closed'), 'success');
      if (onClose) onClose();
    } catch (err) {
      setError(t('doctor.visit.closeFailed') + ': ' + err.message);
      showToast(t('doctor.visit.closeFailed'), 'error');
    } finally {
      setLoading(false);
    }
  }, [visitId, apiFetch, onClose, t, showToast]);
  ...
  const saveVisitDetails = useCallback(async () => {
    try {
      ...
      setIsSaved(true);
      showToast(t('doctor.visit.saved'), 'success');
    } catch (err) {
      setError(t('doctor.visit.saveFailed') + ': ' + err.message);
      showToast(t('doctor.visit.saveFailed'), 'error');
    } finally {
      setLoading(false);
    }
  }, [visitId, chiefComplaint, history, medications, allergies, doctorNotes, dischargeStatus, apiFetch, t, showToast]);
  ...
  const addDrugToVisit = useCallback(async (drugId) => {
    try {
      ...
      await loadVisitDrugs();
      showToast(t('doctor.visit.drugAdded'), 'success');
    } catch (err) {
      setError('Failed to add drug: ' + err.message);
      showToast('Failed to add drug', 'error');
    }
  }, [visitId, apiFetch, loadVisitDrugs, t, showToast]);
  ...
  const removeDrugFromVisit = useCallback(async (drugId) => {
    try {
      ...
      await loadVisitDrugs();
      showToast(t('doctor.visit.drugRemoved'), 'success');
    } catch (err) {
      setError('Failed to remove drug: ' + err.message);
      showToast('Failed to remove drug', 'error');
    }
  }, [visitId, apiFetch, loadVisitDrugs, t, showToast]);
  ...
  const savePrescription = useCallback(async () => {
    ...
    const { duration, durationType, frequency, dose, instruction } = prescriptionData;
    if (!duration || !durationType || !frequency) {
      showToast(t('doctor.visit.fillRequired'), 'warning');
      return;
    }
    ...
    try {
      ...
      showToast(t('doctor.visit.prescriptionSaved'), 'success');
      setShowPrescriptionDialog(false);
      setSelectedDrugId(null);
      setPrescriptionData({ duration: '', durationType: '', frequency: null, dose: '', instruction: '' });
    } catch (err) {
  ...
  }, [visitId, selectedDrugId, prescriptionData, apiFetch, t, showToast]);
  ...
  const showPrescription = useCallback(async (drugId) => {
    try {
      ...
      if (!found) {
        showToast(t('doctor.visit.noPrescription'), 'warning');
        return;
      }
      ...
      const message = `
        ...
        Instruction: ${found.instructions || 'Not specified'}
      `;
      // Use toast for prescription details or a more detailed display
      showToast(`📋 Prescription: ${found.tradeName} - Dose: ${found.dose || 'N/A'}`, 'info');
    } catch (err) {
      setError('Failed to load prescription: ' + err.message);
    }
  }, [visitId, apiFetch, mapDurationType, t, showToast]);
  ...
  const addProcedureToVisit = useCallback(async (procedure) => {
    if (!procedure || selectedProcedures.includes(procedure)) {
      if (selectedProcedures.includes(procedure)) {
        showToast(t('doctor.visit.selectProcedure'), 'warning');
      }
      return;
    }
    ...
    try {
      ...
      await loadSelectedProcedures();
      showToast(t('doctor.visit.procedureAdded'), 'success');
    } catch (err) {
      setError('Failed to add procedure: ' + err.message);
      showToast('Failed to add procedure', 'error');
    }
  }, [visitId, selectedProcedures, apiFetch, loadSelectedProcedures, t, showToast]);
  ...
  const removeProcedureFromVisit = useCallback(async (procedure) => {
    try {
      ...
      await loadSelectedProcedures();
      showToast(t('doctor.visit.procedureRemoved'), 'success');
    } catch (err) {
      setError('Failed to remove procedure: ' + err.message);
      showToast('Failed to remove procedure', 'error');
    }
  }, [visitId, apiFetch, loadSelectedProcedures, t, showToast]);
    ...
    return (
      <div 
        ...
        style={{
          ...
          backgroundColor: toastColors[toast.type] || '#333',
          color: 'white',
          padding: '12px 20px',
          borderRadius: '8px',
  ...
  return (
    <>
      <style>{`
        ...
        .doctor-visit-screen.rtl .toast-notification {
