...
// export default ClaimsTrackingScreen;   11072026  11:20 pm 
...
const styles = `
  ...
  .claims-header .close-btn:hover {
    background: #cbd5e0;
  }

  .claims-header .close-btn:active {
  ...
  .claims-filter-section .search-input {
    flex: 1;
    min-width: 150px;
  }

  .claims-filter-section .action-btn {
    padding: 8px 20px;
    border: none;
    border-radius: 20px;
    font-weight: bold;
  ...
  .claims-filter-section .action-btn:hover {
    transform: scale(1.05);
  }

  .claims-filter-section .action-btn:active {
    transform: scale(0.95);
  }

  .claims-filter-section .action-btn.primary {
    background: #4a90d9;
    color: white;
  }

  .claims-filter-section .action-btn.primary:hover {
    background: #357abd;
  }

  .claims-filter-section .action-btn.success {
    background: #38a169;
    color: white;
  }

  .claims-filter-section .action-btn.success:hover {
    background: #2f855a;
  }

  .claims-filter-section .action-btn.purple {
    background: #6b46c1;
    color: white;
  }

  .claims-filter-section .action-btn.purple:hover {
    background: #553c9a;
  }

  .claims-filter-section .action-btn.dark {
    background: #2d3748;
    color: white;
  }

  .claims-filter-section .action-btn.dark:hover {
    background: #1a202c;
  }
  ...
  .claims-stats-section .stat-item .stat-value {
    font-size: 20px;
    display: block;
  }

  .claims-stats-section .stat-item .stat-label {
    font-weight: normal;
    font-size: 12px;
    color: #718096;
  }
  ...
  .claims-table-header .actions button:active {
    transform: scale(0.95);
  }
  ...
  .claims-table .settle-btn:active {
    transform: scale(0.95);
  }
  ...
  .claims-selection-bar button:active {
    transform: scale(0.95);
  }

  .claims-selection-bar .btn-success {
    background: #38a169;
    color: white;
  }

  .claims-selection-bar .btn-danger {
    background: #e53e3e;
    color: white;
  }
  ...
  .claims-modal.claims-folder-modal {
    max-width: 800px;
  }
  ...
  .claims-modal .form-group .amount-display {
    font-weight: bold;
    font-size: 16px;
    color: #2d3748;
    padding: 8px 0;
  ...
  .claims-modal .form-group .amount-display.outstanding {
    color: #e53e3e;
  }
  ...
  .claims-modal .modal-actions button:active {
    transform: scale(0.95);
  }

  .claims-modal .modal-actions button:disabled {
    opacity: 0.6;
    cursor: not-allowed;
    transform: none;
  }
  ...
  .claims-modal .folder-table {
    width: 100%;
    border-collapse: collapse;
    font-size: 13px;
  }

  .claims-modal .folder-table th {
    padding: 10px;
    text-align: left;
    background: #f8f9fa;
    border-bottom: 2px solid #e2e8f0;
  ...
  .claims-modal .folder-table td {
    padding: 8px 10px;
    border-bottom: 1px solid #e2e8f0;
  }

  .claims-modal .folder-table tr:hover td {
    background: #f7fafc;
  }

  .claims-modal .folder-table .delete-btn {
    background: #e53e3e;
    color: white;
    border: none;
    border-radius: 12px;
  ...
  .claims-modal .folder-table .delete-btn:hover {
    background: #c0392b;
    transform: scale(1.05);
  }
  ...
  @media (max-width: 768px) {
    ...
    .claims-table .checkbox-cell::before {
      content: '';
      display: none;
    }
...
const ClaimsTrackingScreen = ({ loggedUser, lang = 'en', onClose }) => {
  ...
  const exportToCSV = () => {
    ...
    let csv = 'ID,Patient,Doctor,Provider,Insurance Amount,Paid Amount,Discount,Outstanding,Status,Date\n';
    ...
    filteredClaims.forEach(claim => {
      const row = [
        ...
        claim.insuranceAmount.toFixed(2),
        claim.insurancePaidAmount.toFixed(2),
        claim.insuranceDiscount.toFixed(2),
        claim.insuranceOutstanding.toFixed(2),
        claim.insurancePaid ? 'Paid' : 'Unpaid',
        claim.paidAt || '',
      ];
    ...
    return (
      <div className="claims-stats-section">
        <div className="stat-item">
          ...
          <span className="stat-label">{t.claimsCount}</span>
        </div>
        <div className="stat-item" style={{ color: '#e53e3e' }}>
          <span className="stat-value">{formatCurrency(stats.outstanding)}</span>
          <span className="stat-label">{t.column.outstanding}</span>
        </div>
      </div>
    );
  };
    ...
    return (
      <div className="claims-selection-bar">
        ...
        <button
          ...
          style={{ background: 'transparent', color: '#a0aec0', border: 'none', cursor: 'pointer' }}
        >
          ✕
        </button>
      </div>
          ...
          <tbody>
            {filteredClaims.map(claim => (
              <tr key={claim.id}>
                ...
                <td data-label={t.column.id}>{claim.id}</td>
                <td data-label={t.column.patient} style={{ fontWeight: 'bold' }}>{claim.patientName}</td>
                <td data-label={t.column.doctor}>{claim.doctorName}</td>
                <td data-label={t.column.provider}>{claim.insuranceProvider}</td>
                <td data-label={t.column.insurance} className="amount-positive">{formatCurrency(claim.insuranceAmount)}</td>
                <td data-label={t.column.paid} style={{ color: '#38a169' }}>{formatCurrency(claim.insurancePaidAmount)}</td>
                <td data-label={t.column.discount} style={{ color: '#805ad5' }}>{formatCurrency(claim.insuranceDiscount)}</td>
                <td data-label={t.column.outstanding} className={claim.insuranceOutstanding > 0 ? 'amount-negative' : 'amount-positive'}>
                  {formatCurrency(claim.insuranceOutstanding)}
                </td>
                <td data-label={t.column.paidStatus}>
                  <span className={claim.insurancePaid ? 'status-paid' : 'status-pending'}>
                    {claim.insurancePaid ? `✓ ${t.paid}` : `⏳ ${t.pending}`}
                  </span>
                </td>
                <td data-label={t.column.paidAt}>{formatDate(claim.paidAt)}</td>
  ...
  const renderSettleModal = () => {
    ...
    return (
      <div className="claims-modal-overlay" onClick={() => setShowSettleModal(false)}>
        <div className="claims-modal" onClick={e => e.stopPropagation()}>
          ...
          <div className="form-group">
            <label>{t.outstanding}:</label>
            <div className="amount-display outstanding">
              {formatCurrency(currentClaim.insuranceOutstanding)}
            </div>
          </div>
  ...
  const renderFolderModal = () => {
    ...
    return (
      <div className="claims-modal-overlay" onClick={() => setShowFolderModal(false)}>
        <div className="claims-modal claims-folder-modal" onClick={e => e.stopPropagation()}>
          <h3>📂 {t.folders}</h3>
  ...
  const renderCreateFolderModal = () => {
    ...
    return (
      <div className="claims-modal-overlay" onClick={() => setShowCreateFolderModal(false)}>
        <div className="claims-modal" onClick={e => e.stopPropagation()}>
          <h3>📁 {t.createFolderTitle}</h3>
          <p style={{ color: '#4a5568', marginBottom: '15px' }}>
            {t.createFolderHeader}
